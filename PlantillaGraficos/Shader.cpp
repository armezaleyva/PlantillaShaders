...
Shader::Shader(const char* rutaVertexShader,
	...
	ifstream fragmentShaderStream(rutaVertexShader, ios::in);
...
void Shader::verificarCompilacion(GLuint id) {
	...
	if (longitudLog > 0) {
		// Inicia una lista de char con el número de elementos
		// indicados en longitudLog
...
void Shader::verificarVinculacion(GLuint id) {
	...
	if (estadoVinculacion == GL_FALSE) {
		cout << "No se puede validar la vinculación" << endl;
	}
